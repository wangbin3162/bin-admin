  ...
  <div>
    ...
    <page-header-wrap v-if="isEdit" :title="editTitle +' '+ resource.resourceName" show-close
      ...
      <v-edit-wrap transparent>
        <b-collapse-wrap>
          <div slot="title" style="font-size: 16px;padding: 16px;" flex="cross:center">
            <span style="margin-right: 8px;">基本信息</span>
            <b-icon v-if="isFormChange" name="logo-ionic" color="#fea638"/>
          </div>
...
export default {
  ...
  computed: {
    ...
    // 判断是否修改过
    isFormChange() {
      return !(JSON.stringify(this.formBuffer) === JSON.stringify(this.form))
    },
    // 动态列
    dynamicColumns() {
      let ret = []
    ...
    handleModify(row) {
      api.queryOne(row.id, this.resource.resourceKey).then(res => {
        if (res.data.code === '0') {
          ...
          // 缓存进入详情，用于判定是否修改
          this.formBuffer = deepCopy(this.form)
