      ...
      <b-row :gutter="15" style="width:100%;">
        ...
        <b-col span="2">
          <div flex="dir:top main:center cross:center" style="height: 400px;">
            <b-button plain size="small" type="primary" style="width: 95%;margin-bottom: 5px;"
                      @click="autoMap">
              自动映射
            </b-button>
        ...
        <b-col span="12">
          ...
          <b-table :columns="columns3" :data="itemMaps" size="small" max-height="475">
            ...
            <template v-slot:dict="scope">
              <b-button type="primary" size="mini" plain @click="editDict(scope.row)" v-waves
                        v-if="itemMaps[scope.index].type === 'SELECT'">
                添加配置
              </b-button>
            </template>
            <template v-slot:action="scope">
              <b-button type="primary" size="mini" plain @click="cancelOneMap(scope.row)" v-waves>
                取消
              </b-button>
            </template>
          </b-table>
      ...
      <template>
        ...
        <div flex="box:last" v-for="item in dictMaps" :key="item.sourceKey" class="mb-5">
          <b-input class="mr-10" v-model="item.sourceKey"
                   placeholder="请输入键" size="small"></b-input>
          <b-input v-model="item.sourceValue"
                   placeholder="请输入值" size="small"></b-input>
          <!--操作栏-->
          <div style="width: 140px;padding:4px 0 0 10px; text-align: center;">
            <b-button type="danger" size="mini" v-waves icon="ios-remove-circle" transparent
