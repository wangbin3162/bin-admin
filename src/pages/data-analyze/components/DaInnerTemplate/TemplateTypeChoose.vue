<template>
  <b-modal v-model="chooseDialog" title="选择模板类型" width="500px" min-height="400px">
    <b-table :columns="columns" :data="list">
      <!--操作栏-->
      <template v-slot:action="scope">
        <b-button type="text" @click="handleChoose(scope.row)">选择</b-button>
      </template>
    </b-table>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          { title: '模板类型', key: 'tempType', align: 'center' },
    ...
    methods: {
      ...
      searchList() {
        ...
        this.list = [{ tempName: '全文检索', tempType: 'term' }]
      },
      // 选中一个角色
      handleChoose(item) {
        this.chooseDialog = false
        this.$emit('on-choose', { tempName: item.tempName, tempType: item.tempType })
      }
    }
  }
</script>
