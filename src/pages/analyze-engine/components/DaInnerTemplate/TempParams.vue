  ...
  <div class="temp-params">
    ...
    >
      <div class="param-item" v-for="(row,index) in totalData" :key="index">
        ...
        <div class="top" @click="row.expand=!row.expand">
          <div class="front">
            <div class="label">
            ...
            <div class="label" style="width: 15%;">
              <b-tag type="primary" no-border style="margin: 0 5px 0 0;">控件类型</b-tag>
              <span>{{ controlTypeMap[row.controlType] }}</span>
            </div>
            <div class="label" style="width: 15%;">
              <b-tag type="primary" no-border style="margin: 0 5px 0 0;">数据类型</b-tag>
              <span>{{ row.paramType }}</span>
            </div>
          </div>
          <div class="right">
            <b-button type="text" @click.stop="row.expand=!row.expand">{{ row.expand ? '收起' : '编辑' }}</b-button>
            <b-button type="text" text-color="danger" @click.stop="handleRemove(index)">删除</b-button>
          </div>
        </div>
