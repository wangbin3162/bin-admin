    ...
    <page-header-wrap v-show="isNormal">
      <v-table-wrap>
        ...
        <b-table :columns="columns" :data="list" :loading="listLoading">
          <template #viewConfig="{row}">
            <b-button type="text" @click="handleConfigView(row)">视图</b-button>
          </template>
          <!--操作栏-->
...
import ViewConfig from '@/pages/demo/form-making/ViewConfig'

export default {
  name: 'TableDemo',
  components: { ViewConfig, FmContainer },
  mixins: [commonMixin],
  data() {
    return {
      listQuery: {
      ...
      columns: [
        ...
        { title: '视图配置', slot: 'viewConfig', align: 'center', width: 110 },
  ...
  methods: {
    ...
    handleConfigView(row) {
      ...
      this.dialogStatus = 'viewConfig'
      this.$refs.viewConfig.open(row.id === 0 ? testData : testData2)
    },
    // 保存表单
